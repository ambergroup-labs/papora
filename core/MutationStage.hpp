...
static class KnownSyscalls {
 ...
 public:
  ...
  uint32_t get(size_t index) { return vector[index]; }

  // uint32_t has(uint32_t nr) {
  //    return (set.find(nr) != set.end());
  // }

  size_t size(void) { return vector.size(); }

} KnownSyscalls;

const std::unordered_set<uint32_t> ImmutableSyscalls = {
    SYS_open,   SYS_rename,  SYS_mkdir,    SYS_rmdir,     SYS_link,
    SYS_unlink, SYS_symlink, SYS_setxattr, SYS_listxattr, SYS_removexattr};
