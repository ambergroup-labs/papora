...
class SyscallMutator {
 ...
 public:
  ...
  Syscall *getTarget() { return this->target; }
  ...
  virtual uint64_t getStageMax(void) { assert(0 && "unimplemented!"); }
  virtual uint32_t mutate(void) { assert(0 && "unimplemented!"); }
  ...
  virtual bool done(void) { assert(0 && "unimplemented!"); }
};
...
class ReadMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->active_file_fds.empty()) return NULL;
...
class LseekMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    ...
    syscall->add_arg(new Arg(
        whence_options[rand32(0, sizeof(whence_options) / sizeof(uint32_t))],
        0));
...
class Getdents64Mutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->active_dir_fds.empty()) return NULL;
...
class StatMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->avail_files.empty()) return NULL;
  ...
  uint32_t havoc(void) {
    if (program->avail_files.empty()) return 0;
...
class FsyncMutator : public SyscallMutator {
 ...
 public:
  ...
  uint32_t havoc(void) {
    if (program->active_file_fds.empty()) return 0;
...
class FstatMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->active_fds.empty()) return NULL;
  ...
  uint32_t havoc(void) {
    if (program->active_fds.empty()) return 0;
...
class FallocateMutator : public SyscallMutator {
 ...
 private:
  ...
  static const constexpr uint32_t mode_options[6] = {
      FALLOC_FL_KEEP_SIZE,  FALLOC_FL_UNSHARE_RANGE,
...
class MmapMutator : public SyscallMutator {
  ...
  //                   int fd, off_t offset);
 private:
  static const constexpr int32_t prot_options[4] = {PROT_EXEC, PROT_READ,
                                                    PROT_WRITE, PROT_NONE};
  static const constexpr int32_t flags_options[7] = {
      // MAP_SHARED,
      // MAP_SHARED_VALIDATE,
      MAP_PRIVATE, MAP_32BIT,
      // MAP_ANONYMOUS,
      MAP_DENYWRITE,
      // MAP_EXECUTABLE,
      // MAP_FILE,
...
class MunmapMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    ...
    auto it = argMap.find(0);
    if (it == argMap.end()) {
      size_t num_active_bases = program->active_map_base_idx.size();
      map_index = program->active_map_base_idx[rand32(0, num_active_bases)];
    ...
    ret->add_arg(
        new Arg(rand32(1, 64 * PAGE_SIZE), 0));  // Random size should be OK?
 ...
 public:
  ...
  Syscall *createTarget(FileObject *fobj) {
    ...
    if (fobj != NULL) {
      ...
      int64_t path_index = program->create_variable(STRING, path.length() + 1,
                                                    (uint8_t *)(path.c_str()));
...
class RenameMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->avail_files.size() < 2) return NULL;
    ...
    switch (rand32(0, 4)) {
      ...
      case 2 ... 3: {
        ...
        new_path_index = program->create_variable(
            STRING, new_path.length() + 1, (uint8_t *)(new_path.c_str()));
        ret->add_arg(new Arg(new_path_index, 1));
...
class RmdirMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->avail_dirs.size() == 1) return NULL;
...
class UnlinkMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    if (program->avail_non_dirs.empty()) return NULL;
...
class SetxattrMutator : public SyscallMutator {
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    ...
    int64_t value_index = program->create_variable(PUCHAR, value_len, value);
    free(value);
  ...
  virtual void releaseTarget() {
    ...
    program->remove_last_variable();
    program->remove_last_variable();
    if (xattr_create) target_fobj->remove_xattr(new_name);
    SyscallMutator::releaseTarget();
  }
};
 ...
 public:
  ...
  virtual Syscall *createTarget(const ArgMap &argMap) {
    ...
    if (target_fobj->xattrs.size() != 0) {
      ...
      name_index = program->create_variable(STRING, old_xattr_name->size,
                                            old_xattr_name->buffer);
      // side effects
  ...
  virtual void releaseTarget() {
    program->remove_last_variable();
    if (old_xattr_name) target_fobj->add_xattr(old_xattr_name);
    SyscallMutator::releaseTarget();
  }
};
