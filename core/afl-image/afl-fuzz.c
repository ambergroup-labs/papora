...
static void read_testcases(void) {
  ...
  if (fsfuzz_mode && queued_paths > 1)
    PFATAL(
        "[fs-fuzz] file system fuzzing currently only supports 1 test case.");
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if ((res = read(fsrv_st_fd, &status, 4)) != 4) {
      if (stop_soon) return 0;
      RPFATAL(res, "Unable to communicate with fork server (OOM?) (read: %d)",
              res);
    }
  }
...
int main(int argc, char** argv) {
  ...
  if (fsfuzz_mode) {
    ...
    if ((!shm_name && !image_file) || !wrapper_file || !seed_file)
      PFATAL(
          "[fs-fuzz] you must provide image path and wrapper path under file "
          "system fuzzing mode");
  }
  ...
  if (fsfuzz_mode) load_seed_image();
