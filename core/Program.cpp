...
[ syscall_num ]
        [ syscall_nr | syscall_arg1_is_var | syscall_arg1 | ... |
syscall_ret_index ]
[ fileobjs_num ]
        [ path_variable_index
        | type
...
void show_syscall(Program *prog, Syscall *syscall) {
  if (syscall->ret_index != -1) {
    printf("\t%s = syscall(%s",
           (prog->variables[syscall->ret_index])->name.c_str(),
           syscall_name[syscall->nr]);
  } else
...
void Program::show() {
  ...
  for (Variable *v : variables) show_variable(v);

  for (Syscall *syscall : syscalls) show_syscall(this, syscall);
...
void Program::prepare_file_paths() {
  for (FileObject *fobj : avail_files) {
    int64_t path_index = create_variable(STRING, fobj->rel_path.length() + 1,
                                         (uint8_t *)(fobj->rel_path.c_str()));
    file_paths.insert(std::make_pair(fobj, path_index));
  }
}

int64_t Program::create_variable(uint8_t type, uint32_t size, uint8_t *value,
                                 uint8_t kind) {
...
static inline void _readmem(void *dest, void *src, uint32_t *ppos,
                            uint32_t len) {
  uint32_t pos = *ppos;
...
static inline void _writemem(void *dest, void *src, uint32_t *ppos,
                             uint32_t len) {
  uint32_t pos = *ppos;
...
uint32_t Program::serialize(uint8_t *buf, uint32_t len) {
  ...
  for (Variable *v : variables) {
    ...
    if (v->is_pointer()) {
      ...
      if (has_value) _writemem(buf, (void *)v->value, &pos, v->size);
    }
...
/******************** destructor **********************/
Variable::~Variable() {
  if (is_pointer() && value) free((void *)value);
}
