...
static u8 fuzz_one(char** argv) {
  ...
  if (!fsfuzz && !dumb_mode && !queue_cur->trim_done) {
...
fsfuzz_stage:
  ...
  /*
    stage_short = "args";
    stage_max = mutate_args_init(out_buf, len);
    stage_name = "syscall-args";
    ...
    for(stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        ...
        mutate_args(out_buf, len);
        if(common_fuzz_stuff(argv, out_buf, len)) goto abandon_entry;
        //memcpy(out_buf, in_buf, len);
        // mutator keeps the original internally
    }

    mutate_args_fini();
    memcpy(out_buf, in_buf, len); //restore
  */
  ...
  mutate_havoc_init(out_buf, len, stage_max);

  stage_val_type = STAGE_VAL_NONE;
  u32 init_queued = fsfuzz_queued = queued_paths;

  u8* temp_buf = (u8*)malloc(MAX_FILE);
  u32 temp_len_fs = 0;

  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    temp_len_fs = mutate_havoc(temp_buf, MAX_FILE, MUTATE);

    if (common_fuzz_stuff(argv, temp_buf, temp_len_fs)) goto abandon_entry;
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    temp_len_fs = mutate_havoc(temp_buf, MAX_FILE, GENERATE);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q:u:k")) > 0)
