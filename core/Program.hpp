...
struct Variable {
  ...
  ~Variable();
  bool is_pointer() { return type == PUCHAR || type == STRING; }
};
...
struct Syscall {
  ...
  Syscall(const Syscall &syscall)
    ...
    for (auto a : syscall.args) args.push_back(new Arg(*a));
  }
  ...
  void add_arg(Arg *arg) { args.push_back(arg); }
};
...
struct Program {
  ...
  std::map<FileObject *, int64_t>
      file_paths;  // store variables for path of all file objects
  ...
  // create variable
  int64_t create_variable(uint8_t type, uint32_t size = 0,
                          uint8_t *value = NULL, uint8_t fd_type = UNKNOWN);
  ...
  void remove_syscall(Syscall *syscall) {
    syscalls.erase(std::remove(syscalls.begin(), syscalls.end(), syscall),
                   syscalls.end());
  }
  ...
  void remove_file(FileObject *fobj) {
    avail_files.erase(std::remove(avail_files.begin(), avail_files.end(), fobj),
                      avail_files.end());
    if (fobj->type == I_DIR)
      remove_dir(fobj);
    else
  ...
  void remove_dir(FileObject *fobj) {
    avail_dirs.erase(std::remove(avail_dirs.begin(), avail_dirs.end(), fobj),
                     avail_dirs.end());
  }

  void remove_non_dir(FileObject *fobj) {
    avail_non_dirs.erase(
        std::remove(avail_non_dirs.begin(), avail_non_dirs.end(), fobj),
        avail_non_dirs.end());
  }
  ...
  void remove_fd(int64_t fd_index) {
    ...
    active_fds.erase(
        std::remove(active_fds.begin(), active_fds.end(), fd_index),
        active_fds.end());
  }

  void remove_file_fd(int64_t fd_index) {
    active_file_fds.erase(
        std::remove(active_file_fds.begin(), active_file_fds.end(), fd_index),
        active_file_fds.end());
  }

  void remove_dir_fd(int64_t fd_index) {
    active_dir_fds.erase(
        std::remove(active_dir_fds.begin(), active_dir_fds.end(), fd_index),
        active_dir_fds.end());
  }
  ...
  void mark_base_unmapped(int64_t map_index) {
    ...
    auto it = std::find(active_map_base_idx.begin(), active_map_base_idx.end(),
                        map_index);
