...
void create_corpus(Image *image, char *folder) {
  ...
  for (FileObject *fobj : image->file_objs) {
    ...
    std::string path =
        std::string(folder) + "/open_read" + std::to_string(cnt++);
    if (!mem)
      program->serialize(path.c_str());
    else {
      char *buffer = (char *)malloc(TEST_MEM_SIZE);
      uint32_t new_len =
          program->serialize((uint8_t *)buffer, (uint32_t)TEST_MEM_SIZE);
...
int main(int argc, char *argv[]) {
  ...
  if (image) create_corpus(image, argv[2]);
