...
static void activity(char *mpoint) {
  ...
  if (fd >= 0) {
    void *mem =
        lkl_sys_mmap(NULL, 4096, PROT_READ, MAP_PRIVATE | MAP_POPULATE, fd, 0);

    if (mem != MAP_FAILED) lkl_sys_munmap((unsigned long)mem, 4096);
  ...
  if (fd >= 0) {
    ...
    lkl_sys_fallocate(fd, FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE, 0,
                      4243261);
    lkl_sys_fallocate(fd, FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE, -95713,
                      38447);
    lkl_sys_fallocate(fd, FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE, 18237,
                      -9173);
...
void *userfault_init(void *image_buffer, size_t size) {
  ...
  if (ioctl(uffd, UFFDIO_API, &uffdio_api) == -1) errExit("ioctl-UFFDIO_API");

  void *buffer = mmap(NULL, len, PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (buffer == MAP_FAILED) errExit("mmap");
...
int main(int argc, char **argv) {
  ...
  if (argp_parse(&argp_fsfuzz, argc, argv, 0, 0, &cla) < 0) return -1;
  ...
  else if (!strcmp(cla.fsimg_type, "ntfs3"))
  ...
  if (!cla.fsimg_path) {
    ...
    image_buffer =
        mmap(0, st.st_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
    close(fd);
    size = st.st_size;
  }
