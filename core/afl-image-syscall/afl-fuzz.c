...
EXP_ST u8 *in_dir,   /* Input directory with test cases  */
    ...
    *image_file,     /* for fs fuzzing                   */
    *wrapper_file,   /* for fs fuzzing                   */
    *seed_file,      /* for fs fuzzing                   */
    *mutate_file,    /* for fs fuzzing                   */
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  if (fsfuzz_mode) {
    ...
    wrapper_decompress(mem, mutate_file, meta_size);
    //if (image_file) wrapper_overwrite_to_file(image_file, 0, meta_size);
    //if (image_file) wrapper_sync_to_file(image_file);
...
size_t mutate_file_size;
static void load_seed_image(void) {
  void* buffer = NULL;
  ...
  int ifd = open(image_file, O_RDWR | O_CREAT, 0666);

  mutate_file_size = st.st_size;  
  if (sendfile(ifd, sfd, NULL, mutate_file_size) < 0)
    PFATAL("sendfile failed");

  /* mmap for mutate_file */
  mutate_file = mmap(NULL, mutate_file_size, PROT_READ | PROT_WRITE, MAP_SHARED, ifd, 0);
  if (mutate_file == MAP_FAILED)
    FATAL("mmap for %s failed", image_file);
  ...
  if (shm_name) {
    ...
    if (fd < 0) PFATAL("Open shm %s failed.", shm_name);
    OKF("Open shm %s success.", shm_name);

    if (ftruncate(fd, st.st_size) < 0)
      PFATAL("ftruncate on shm %s failed.", shm_name);

    buffer = mmap(NULL, st.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (buffer == MAP_FAILED) PFATAL("Mapping shm failed.");
    OKF("Map shm %s at %p size: 0x%lx.", shm_name, buffer, st.st_size);

    close(fd);
  } else {
    buffer = mmap(NULL, st.st_size, PROT_READ | PROT_WRITE,
...
static void load_syscalls(void) {
  struct dirent** nl;
  struct stat st;
  s32 nl_cnt;
  u32 i;

  nl_cnt = scandir(syscall_in_dir, &nl, NULL, alphasort);

  ACTF("Scanning '%s'...", syscall_in_dir);

  if (nl_cnt < 0) {
    if (errno == ENOENT || errno == ENOTDIR)
      SAYF("\n" cLRD "[-] " cRST
           "The syscall input directory does not seem to be valid - try again. "
           "\n");

    PFATAL("Unable to open '%s'", in_dir);
  }

  u8* seed_meta_fn = alloc_printf("%s/seed.meta", in_dir);
  if (lstat(seed_meta_fn, &st) || access(seed_meta_fn, R_OK))
    PFATAL("Unable to access '%s'", seed_meta_fn);
  meta_size = st.st_size;
  int meta_fd = open(seed_meta_fn, O_RDONLY);

  for (i = 0; i < nl_cnt; i++) {
    u8* syscall_fn = alloc_printf("%s/%s", syscall_in_dir, nl[i]->d_name);
    if (lstat(syscall_fn, &st) || access(syscall_fn, R_OK))
      PFATAL("Unable to access '%s'", syscall_fn);

    /* This also takes care of . and .. */
    if (!S_ISREG(st.st_mode) || !st.st_size ||
        strstr(syscall_fn, "/README.txt")) {
      ck_free(syscall_fn);
      continue;
    }
    ...
    u8* testcase_fn = alloc_printf("%s/%s", in_dir, nl[i]->d_name);
  ...
  close(meta_fd);
  if (unlink(seed_meta_fn)) PFATAL("Unable to unlink '%s'", seed_meta_fn);
...
static u8 fuzz_one(char** argv) {
  ...
  if (fsfuzz_mode || skip_deterministic || queue_cur->was_fuzzed ||
...
retry_splicing:
  ...
  if (!fsfuzz_mode && use_splicing && splice_cycle++ < SPLICE_CYCLES &&
...
fsfuzz_stage:
  ...
  stage_short = "fs-havoc-mutate";
  stage_max = MUTATE_HAVOC_CYCLES * perf_score / havoc_div / 100;
  if (stage_max < HAVOC_MIN) stage_max = HAVOC_MIN;
  stage_name = "fs-havoc-mutate";

  mutate_havoc_init(out_buf + meta_size, len - meta_size, stage_max);

  stage_val_type = STAGE_VAL_NONE;
  u32 init_queued = fsfuzz_queued = queued_paths;
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (common_fuzz_stuff(argv, new_buf, meta_size + fs_buf_len))
      goto fsfuzz_fini;
  ...
  if (queued_paths > init_queued) goto fsfuzz_fini;

  stage_short = "fs-havoc-generate";
  stage_max = GENERATE_HAVOC_CYCLES * perf_score / havoc_div / 100;
  if (stage_max < HAVOC_MIN) stage_max = HAVOC_MIN;
  stage_name = "fs-havoc-generate";

  stage_val_type = STAGE_VAL_NONE;

  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    fs_buf_len = mutate_havoc(new_buf + meta_size, MAX_FILE, GENERATE);
...
// memcpy(out_buf + meta_size, in_buf + meta_size, len - meta_size); //restore
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q:u:g:kb:s:e:y:")) >
         0)
    ...
    switch (opt) {
      ...
      case 'u': /* CPU# */
        ...
        if (sscanf(optarg, "%u", &cpu_id) != 1) PFATAL("Invalid CPU#");
  ...
  if (fsfuzz_mode) {
    if ((!shm_name && !image_file) || !wrapper_file || !seed_file ||
        !syscall_in_dir)
      PFATAL("[fs-fuzz] missing arguments for file system fuzzing mode.");
  }
