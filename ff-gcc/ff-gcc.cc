...
void run(list<string> &params) {
  ...
  if (!args) err(1, "failed to allocate memory");
...
string get_output(string cmd) {
  ...
  if (!fp) err(1, "failed to run: %s", cmd.c_str());
...
void adjust_clang_flags(list<string> &params) {
  ...
  params.push_back("-Wno-duplicate-decl-specifier");
  ...
  //params.push_back("-fsanitize=kernel-address -mllvm -asan-stack=0");
  params.push_back("-fsanitize=kernel-address");
  params.push_back("-mllvm");
  params.push_back("-asan-stack=0");
...
int main(int argc, char *argv[]) {
  if (argc < 2) err(1, "[usage] %s [dir]", argv[0]);
  ...
  if (clang) adjust_clang_flags(params);
  ...
  if (clang) {
    ...
    for (auto &i : params) {
      if (i.find(argv[1]) != string::npos) fout << "> " << i << endl;
    }
