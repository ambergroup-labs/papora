...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(argv[1], "test")) {
    ...
    ntfs_fuzzer.compress(in_path, image_buffer, meta_path);
    ...
    ntfs_fuzzer.decompress(tmp_buffer);

    ntfs_fuzzer.sync_to_file(out_path);
    ...
    void *image_buffer = malloc(st.st_size);
    ntfs_fuzzer.compress(in_path, image_buffer);
