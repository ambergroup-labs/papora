...
class fsfuzzer {
 public:
  ...
  virtual void compress(const char *in_path, void *buffer,
                        const char *meta_path = NULL) {}

  virtual void decompress(const void *meta_buffer, size_t meta_len,
                          bool checksum = true) {}

  virtual void general_decompress(const void *meta_buffer, size_t meta_len,
                                  bool checksum = true) {}

  void general_compress(const char *in_path, void *buffer,
                        const char *meta_path = NULL) {
    void *zero;
    struct stat st;
    ...
    if (in_image_fd < 0) FATAL("[-] image %s compression failed.", in_path);
    ...
    if (generate_meta_image) {
      ...
      if (meta_image_fd < 0) FATAL("[-] image %s compression failed.", in_path);
    }

    if (!release_metadata(meta_blocks, meta_image_fd, true))
      FATAL("[-] image %s compression failed.", in_path);

    if (generate_meta_image) close(meta_image_fd);
  ...
  void overwrite_to_file(const char *out_path, size_t off = 0, size_t len = 0) {
    int fd = open(out_path, O_CREAT | O_RDWR, 0666);
    if (write(fd, (uint8_t *)image_buffer_ + off, len) != len)
      FATAL("[-] image sync to file %s failed.", out_path);
    close(fd);
  }
  ...
  bool release_metadata(std::set<uint64_t> &meta_blocks, int meta_image_fd,
    ...
    do {
      ...
      for (it++; it != meta_blocks.end(); it++) {
        ...
        if (cur_offset + block_size_ ==
            (in_block ? (*it) * block_size_ : (*it))) {
          extent.second += block_size_;
      ...
      // std::cout << "offset: " << extent.first << "size: " << extent.second <<
      // std::endl;

      if (meta_image_fd > 0) {
        if (write(meta_image_fd, (char *)image_buffer_ + extent.first,
